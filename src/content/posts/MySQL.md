...
**为什么 MySQL 选择 B+Tree 而不是 Hash？**
Hash 不支持范围查询、不支持排序、联合索引必须全部列匹配。数据库场景中范围查询和排序极其常见，Hash 不适合作为通用索引结构。

> InnoDB 存在「自适应哈希索引」（Adaptive Hash Index），实际上是 B+Tree + Hash 结合，每个哈希桶内是一个小型 B+Tree。

**为什么 B+Tree 优于 B Tree？**
- B+Tree 只有叶子节点存数据，非叶子节点能容纳更多索引键，树更矮，I/O 更少
- 每次查询必须从根走到叶子，查询性能更稳定
...
| 类型 | 说明 |
|------|------|
| B+Tree 索引 | 默认和最常用，MyISAM 和 InnoDB 都使用 |
| Hash 索引 | 仅 MEMORY 引擎支持，适合等值查询 |
| Full-Text 索引 | 全文索引，`CHAR`/`VARCHAR`/`TEXT` 可用 |
| RTree 索引 | 空间索引，仅 geometry 数据类型 |
